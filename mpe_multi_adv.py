"""DiffFP trainer for the MPE-Adversary experiment.

Requires the DiffFP PettingZoo fork (github.com/Aku02/PettingZoo), whose
simple_adversary_v3 is fixed to 1 adversary vs 2 good agents with 3 landmarks,
partial observations (adversary 10-dim / good agents 12-dim, positions only,
goal hidden from the adversary) and a sparse zero-sum team reward
(reach/catch/draw, threshold 0.5).
"""
import argparse
import numpy as np
import torch
...
def readParser():
    ...
    parser.add_argument('--num_steps', type=int, default=1024, metavar='N',
...
class FSPTrainer:
    ...
    def _initialize_environment(self):
        # Note: using the parallel API here. The DiffFP PettingZoo fork ignores N
        # (world is fixed to 1 adversary, 2 good agents, 3 landmarks).
    ...
    def _initialize_logger(self):
        return wandb.init(
            project="fsp_training_mpe_adv",
            config=self.args.__dict__,
            name=f"{self.args.algo}_mpe_adv",
        )

    def _initialize_agents(self):
        # Using the unwrapped environment to get observation and action spaces
        obs_space = self.env.unwrapped.observation_spaces['agent_0']
        ...
        while steps <= self.args.num_steps:
            ...
            while self.env.agents:
                ...
                if self.args.algo == 'qvpo':
                    ...
                    trained_agent.train(self.global_step, 1, batch_size=self.args.batch_size, log_writer=self.logger)
                else:
                    trained_agent.train(1, batch_size=self.args.batch_size, log_writer=self.logger)
    ...
    def evaluate(self, iteration):
        ...
        video_folder = "mpe_adv_landmark_1v2_original"
    ...
    def populate_buffer(self, seed=42, gamma=0.9, buffer_size=100000):
        ...
        while steps <= buffer_size:
            ...
            while self.env.agents:
                ...
                for agent, obs in observations.items():
                    obs = pad_obs(obs)
                    actions[agent] = self.rescale_action(self.env.action_space(agent).sample())
                new_obs, rewards, terminations, truncations, infos = self.env.step(actions)
    ...
    def main(self):
        ...
        self.evaluate(fsp_iter)
        self.agent1.save_model(dir='mpe_save_model_1')
        self.agent2.save_model(dir='mpe_save_model_2')
