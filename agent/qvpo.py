...
from agent.diffusion_qvpo import Diffusion
...
class QVPO(object):
    ...
    def action_gradient(self, batch_size, log_writer, return_mean_std=False):
        ...
        q = torch.min(q1, q2)
        # detach: these feed the running Q statistics used for loss weighting;
        # without detach the stats retain the autograd graph across iterations.
        mean = q.mean().detach()
        std = q.std().detach()
